...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  try {
    ...
    global.db.data.users[m.sender].lastVideoChat = m.chat
...
👇 *Selecciona una opción abajo:*
    ...
    const buttons = [
      { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "🎵 Audio MP3", id: "1" }) },
      { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "🎬 Video MP4", id: "2" }) },
      { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "📄 Audio Doc", id: "3" }) },
      { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "📁 Video Doc", id: "4" }) },
      { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "🎙️ Nota de Voz", id: "5" }) }
    ]
...
handler.before = async function (m, { conn }) {
  ...
  if (user.lastVideoChat && user.lastVideoChat !== m.chat) return
